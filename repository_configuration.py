...
for repo in repos_list:
    ...
    if EXPECTED_CONFIGURATION['deleteBranchOnMerge'] != repo_view['deleteBranchOnMerge']: 
        print(f"🛠 Fixing the 'deleteBranchOnMerge' flag in the repo {repo['name']}")
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--delete-branch-on-merge={EXPECTED_CONFIGURATION['deleteBranchOnMerge']}")
    if EXPECTED_CONFIGURATION['hasDiscussionsEnabled'] != repo_view['hasDiscussionsEnabled']: 
        print(f"🛠 Fixing the 'hasDiscussionsEnabled' flag in the repo {repo['name']}")
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--enable-discussions={EXPECTED_CONFIGURATION['hasDiscussionsEnabled']}")
    if EXPECTED_CONFIGURATION['hasProjectsEnabled'] != repo_view['hasProjectsEnabled']: 
        ...
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--enable-projects={EXPECTED_CONFIGURATION['hasProjectsEnabled']}")
    if EXPECTED_CONFIGURATION['mergeCommitAllowed'] != repo_view['mergeCommitAllowed']: 
        ...
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--enable-merge-commit={EXPECTED_CONFIGURATION['mergeCommitAllowed']}")
    if EXPECTED_CONFIGURATION['rebaseMergeAllowed'] != repo_view['rebaseMergeAllowed']: 
        ...
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--enable-rebase-merge={EXPECTED_CONFIGURATION['rebaseMergeAllowed']}")
    if EXPECTED_CONFIGURATION['squashMergeAllowed'] != repo_view['squashMergeAllowed']: 
        ...
        GhCliAdapter.repo_edit(args.owner, repo["name"], f"--enable-squash-merge={EXPECTED_CONFIGURATION['squashMergeAllowed']}")
    ...
    for repo_parameter_name in EXPECTED_CONFIGURATION:
        if EXPECTED_CONFIGURATION[repo_parameter_name] != repo_view[repo_parameter_name]:
            print(f"🚧 Mismatching parameter '{repo_parameter_name}' in {repo['name']}: {EXPECTED_CONFIGURATION[repo_parameter_name]} != {repo_view[repo_parameter_name]}") 
