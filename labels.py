...
EXPECTED_LABELS_INFO = [
    ...
    {"name": "invalid", "color": "e4e669", "description": "This doesn't seem right"},
    {
        "name": "python",
        "color": "2b67c6",
        "description": "This issue or pull request is related to Python code",
    },
    {
        "name": "question",
        "color": "d876e3",
...
def get_label_info_in_list(label_name, label_list):
    matching_labels = list(
        filter(
            lambda label, label_name=label_name: label_name == label["name"], label_list
        )
    )
    if len(matching_labels) == 1:
        ...
        print(
            f"Error: several matching labels for the label '{label_name}' in {label_list}: {matching_labels}"
        )
        sys.exit(1)
...
for repo in repos_list:
    ...
    for repo_label in repo_labels_info:
        if not is_label_in_list(repo_label["name"], EXPECTED_LABELS_INFO):
            ...
            print(repo_label)
            continue
        matching_label_info_expected = get_label_info_in_list(
            repo_label["name"], EXPECTED_LABELS_INFO
        )
        if matching_label_info_expected["color"] != repo_label["color"]:
            print(
                f"🛠 Fixing the color of the label '{repo_label['name']}' in the repo {repo['name']}"
            )
            GhCliAdapter.set_label_color(
                args.owner,
                repo["name"],
                repo_label["name"],
                matching_label_info_expected["color"],
            )
        if matching_label_info_expected["description"] != repo_label["description"]:
            print(
                f"🛠 Fixing the description of the label '{repo_label['name']}' in the repo {repo['name']}"
            )
            GhCliAdapter.set_label_description(
                args.owner,
                repo["name"],
                repo_label["name"],
                matching_label_info_expected["description"],
            )
